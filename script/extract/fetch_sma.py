...
def get_max_date(symbol, engine):
    try:
        with engine.connect() as connection:
            query = text("SELECT MAX(date) FROM alphavantage_sma WHERE symbol = :symbol")
...
# ============================================================
def fetch_sma_from_api(symbol, api_key):
    url = f"https://www.alphavantage.co/query?function=SMA&symbol={symbol}&interval=daily&time_period=50&series_type=open&apikey={api_key}"
    try:
...
# ============================================================
def transform_sma_data(raw_data, symbol):
    if not raw_data:
        return pd.DataFrame()
    ...
    df.rename(columns={'index': 'date', 'SMA': 'sma'}, inplace=True)
    df['symbol'] = symbol
...
# ============================================================
def load_sma_to_db(engine, df, symbol):
    ...
    with engine.connect() as connection:
        metadata = MetaData()
        table = Table('alphavantage_SMA', metadata, autoload_with=engine)
        ...
        try:
            ...
            query = text("SELECT COUNT(*) FROM alphavantage_SMA WHERE symbol = :symbol")
...
def fetch_sma_data(mode='daily'):
    ...
    print(f"Bắt đầu fetch dữ liệu SMA cho các mã: {tickers}")
    
    for symbol in tickers:
        print(f"=== [{symbol}] BẮT ĐẦU ===")
        raw_data = fetch_sma_from_api(symbol, api_key)
        
        if not raw_data:
            print(f"[{symbol}] Không có dữ liệu. Bỏ qua.")
            continue

        df = transform_sma_data(raw_data, symbol)
        ...
        if df.empty:
            ...
            load_sma_to_db(engine, df, symbol)
        
        # Alpha Vantage API: 5 requests/phút -> nghỉ 15s/mã
        sleep(15)

    print("✅ Hoàn tất quá trình fetch SMA.")

# ============================================================
# 7️⃣ CHẠY TỪ DÒNG LỆNH
# ============================================================
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch SMA data from Alpha Vantage API.")
    parser.add_argument(
        "--mode",
        type=str,
        default="daily",
